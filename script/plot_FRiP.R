...
# read data 
read_in_peak <- read.table(paste0(file_dir,'/', save_name, '_read_in_peak.txt'))
read_in_peak

total_reads <- read.table(paste0(file_dir,'/', save_name, '_total_reads.txt'))
total_reads

sample_IDs <- read.table(paste0(file_dir,'/', save_name, '_sample_IDs.txt'))
sample_IDs
...
# frip_dataframe
reso <- 600
png(filename = paste0(fig_dir, "/", date(),save_name,"-FRIP.png"), width = 1200 * reso/72, height = 700 * reso/72, units ="px", res = reso)
par(mar = c(5,15,5,5))
par(mfrow=c(1,2))
