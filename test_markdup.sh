#!/bin/bash
#SBATCH --job-name=check_markdupp
#SBATCH --output=markdup_test_error.out
#SBATCH --time=96:0:0
#SBATCH --ntasks=1
#SBATCH --mem=90G
...
java -Xmx2g -Djava.io.tmpdir=$new_tmp_dir -jar $picardTool MarkDuplicates \
	...
	-METRICS_FILE ${sample_rmdup}/${sample_ID}_marked_dup_metrics.txt \
	-TMP_DIR $new_tmp_dir

  echo "finish mark dup for $sample_ID at $(date)"
